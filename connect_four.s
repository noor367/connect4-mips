...
assert_board_dimension__body:
        ...
        blt     $a0, $a1, assert_board_small                    # if dimension < min, goto assert_board_small
        bgt     $a0, $a2, assert_board_large                    # if dimension > max, goto assert_board_large
...
initialise_board:
	...
	#
	# Frame:    [$ra]
	# Uses:     [$v0,$a0,$t0,$t1,$t3,$t4,$t5,$t6]
	# Clobbers: [$v0,$a0,$t0,$t1,$t3,$t4,$t5,$t6]
	#
	# Locals:
	#   - $t0: int board_width
	#   - $t1: int board_height
...
check_winner:
	...
	#
	# Frame:    [$ra]
	# Uses:     [$v0,$a0,$a1,$a2,$a3,$s0,$s1,$t0,$t1]
	# Clobbers: [$v0,$a0,$a1,$a2,$a3,$t0,$t1]
	#
	# Locals:
	#   - $t0: int row 
	#   - $t1: int 
...
check_line:
	...
	#
	# Frame:    [$ra]
	# Uses:     [$v0,$s0,$s1,$s2,$s3,$t0,$t1,$t3,$t4,$t5]
	# Clobbers: [$v0,$t0,$t1,$t3,$t4,$t5]
	#
	# Locals:
	#   - $s0: int board_width
	#   - $s1: int board_height
	...
	#   - $s3: char EMPTY_CELL
	#   - $t0: int offset 
	#   - $t1: int row = start_row + offset_row;
	#   - $t2: int col = start_col + offset_col;    
	#
	# Structure:
	#   check_line
	#   -> [prologue]
...
check_line__body:
        mul     $t0, $a0, MAX_BOARD_WIDTH                       # $t0 = start_row * board_width;
        add     $t0, $t0, $a1                                   # $t0 = $t0 + start_col; 
        
        lb      $s2, board($t0)                                 # first_cell = board[start_row][start_col];
        la      $s3, CELL_EMPTY                                 # $t3 = CELL_EMPTY;
        beq     $s2, $s3, check_line__epilogue                  # return WINNER_NONE;
        
        add     $t1, $a0, $a2                                   # $t1: int row = start_row + offset_row;
        add     $t2, $a1, $a3                                   # $t2: int col = start_col + offset_col;    
                
        li      $t3, 0                                          # $t3: int i = 0;
        
        li      $t4, CONNECT
        subu    $t4, $t4, 1                                     # $t4 = CONNECT - 1
        
check_line_loop:
        bge     $t3, $t4, check_line_end                # while (i < CONNECT - 1)
        
        blt     $t1, 0, check_line_none                 # if (row < 0
        blt     $t2, 0, check_line_none                 # || col < 0) return WINNER_NONE;
       
        bge     $t1, $s1, check_line_none               # if (row >= board_height               
        bge     $t2, $s0, check_line_none               # || col >= board_width) return WINNER_NONE;
        
        mul     $t5, $t1, $s0                           # $t5 = row * board_width;
        add     $t5, $t5, $t2                           # $t5 = $t5 + col; 
        
        lb      $s4, board($t5)                         # char cell = board[row][col];
        
        bne     $s4, $s2, check_line_none               # if (cell != first_cell) return WINNER_NONE;
...
check_line_end:
        beq     $t0, CELL_RED, check_line_red                   # if (first_cell == CELL_RED) return WINNER_RED;
        beq     $t0, CELL_YELLOW, check_line_yellow             # if (first_cell == CELL_YELLOW) return WINNER_YELLOW;
