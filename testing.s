...
play_turn_valid_red:
        la      $s4, CELL_RED                   # $s3 = CELL_RED;
        sb      $s4, board($t2)                 # board[target_row][target_col] = CELL_RED;
...
check_winner_loop:
	...
	move    $s2, $v0
	beq     $s2, WINNER_RED, red_wins                       
	beq     $s2, WINNER_YELLOW, yellow_wins                 # if (check != WINNER_NONE)
